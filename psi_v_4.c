...
/* MinGW forwards printf to msvcrt, which does not understand %lld, unless the
 * ANSI-conforming implementation is selected.  PSI numbers are 64-bit, so ask
 * for it before <stdio.h> is pulled in. */
...
#ifdef _WIN32
#  include <io.h>
#  define psi_isatty(fd) _isatty(fd)
#else
#  include <unistd.h>
#  define psi_isatty(fd) isatty(fd)
#endif
...
/* The three singletons above must never be freed.  Immortality is explicit --
 * ref and unref both leave them alone -- rather than relying on a refcount so
 * large it cannot reach zero, because any such sentinel depends on the width
 * of `long`, which is 32 bits on Windows and on 32-bit targets.  LONG_MAX/2 is
 * merely a marker here, not headroom: it is representable in every conforming
 * `long` (which is at least 32 bits) and is never counted up or down. */
#define IMMORTAL_RC (LONG_MAX / 2)

static PVal *ref(PVal *v) { if (v && v->rc < IMMORTAL_RC) v->rc++; return v; }

static void unref(PVal *v) {
    /* Iterative down the list spine so that freeing a long list cannot blow
     * the C stack. */
    while (v) {
        if (v->rc >= IMMORTAL_RC) return;     /* NIL, #t, #f */
        if (--v->rc > 0) return;
        PVal *next = NULL;
        switch (v->t) {
...
static const BuiltinDef builtins[] = {
    ...
    { "head", bi_head }, { "tail", bi_tail },
    { "not", bi_not }, { "type", bi_type },
...
/* Evaluated at startup, ahead of any __start.psi.
 *
 * `list` deliberately lives here rather than in the builtin table above.  The
 * spec sanctions defining it in PSI -- "(def list (& xs) xs)" -- but it also
 * says builtin names cannot be rebound globally, so making it a builtin would
 * make the spec's own suggested definition throw protected-symbol.  Defining
 * it here is the configuration the spec blesses: it prints as a closure
 * instead of $builtin{list} ("this is acceptable") and user code may redefine
 * it freely.
 *
 * `length` is not in the spec at all -- it is an extension the spec's own
 * `let` macro happens to call -- so user code must be able to own that name
 * too.  It is written iteratively so that it costs no C stack on long lists. */
...
static void run_prelude(void) {
    PVal *r = run_source(PRELUDE, "<prelude>");
    if (is_err(r)) {
        fputs("psi: internal error in prelude: ", stderr);
        fwrite_val(stderr, r);
        fputc('\n', stderr);
    }
    unref(r);
...
static void globals_init(void) {
    ...
    NIL->rc = TRUE_V->rc = FALSE_V->rc = IMMORTAL_RC;
...
int main(int argc, char **argv) {
    globals_init();
    run_prelude();                              /* built-in PSI prelude */
    run_file("__start.psi", 1);                 /* optional user prelude */
